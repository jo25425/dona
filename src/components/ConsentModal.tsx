...
export default function Instructions() {
    ...
    const handleAgree = () => {
        ...
        window.location.href = isDonorSurveyEnabled && donorSurveyLink
        ...
        <div>
            ...
            {idInputMethod !== IdInputMethod.AUTOMATED && (
                <Dialog open={dialogOpen} onClose={handleDialogClose}>
                    <DialogTitle>{idInputMethod === IdInputMethod.SHOW_ID ? donor('remember') : donor('enter')}</DialogTitle>
                    <DialogContent>
                        {idInputMethod === IdInputMethod.SHOW_ID && (
                            <Typography variant='body1' sx={{ my: 2, fontWeight: 'bold', textAlign: 'center' }}>{generatedId}</Typography>
                        )}
                    ...
                    <DialogActions sx={{mr: 2, mb: 2}}>
                        <Stack spacing={2} direction='row' sx={{ justifyContent: 'center' }}>
                            <Box><Button onClick={handleDialogClose}>{actions('close')}</Button></Box>
                            <Box><Button onClick={handleAgree} variant='contained'>{actions('next')}</Button></Box>
                        </Stack>
                    </DialogActions>
                </Dialog>
            )}
