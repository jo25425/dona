...
const FullSizeModal: React.FC<FullSizeModalProps> = ({ open, onClose, children, ariaLabel, onAgree }) => {
    ...
    return (
        ...
        >
            <StyledModalBox>
                {/* Optional aria label */}
                {ariaLabel && <Box id="modal-title" sx={{ display: "none" }}>{ariaLabel}</Box>}
                ...
                {onAgree ? (
                    <Stack spacing={2} direction="row" sx={{ justifyContent: "center" }}>
                        ...
                        <Button variant="contained" onClick={onAgree}>{actions("agree")}</Button>
                    </Stack>
                ) : (
                    <Box display="flex" justifyContent="right">
                        <Button variant="contained" onClick={onClose}>{actions("close")}</Button>
                    </Box>
                )}
            </StyledModalBox>
        </Modal>
